...
class SystemHandlePointer : public System, public Handle<SystemImpl>{
    public:
    SystemHandlePointer(const string name="NoName", const double value=0.0){
        pImpl_->setName(name);
        pImpl_->setValue(value);
    }

    virtual ~SystemHandlePointer() {}
